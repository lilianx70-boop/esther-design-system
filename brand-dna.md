...
| 色名 | 色值 | 用途 |
|------|------|------|
| 绿 | `#91BB7C` | 主色调、标题、超链接、重点标记 |
| 奶黄 | `#EFEFB9` | 强调、装饰、badges、连接线、高亮 |
| 粉 | `#FFDAE1` | 点缀、高亮下划线、CTA、标签 |

三色比例原则：主色60% · 强调色25% · 点缀色15%（点缀色永远是点缀，不做主色）
...
- 背景永远偏暖：`#FAFADE`（浅奶基底，主背景）、`#FDF0F3`（浅粉柔底，次要区域）
...
| 类型 | 禁止 |
|------|------|
| 配色 | 蓝紫渐变、cyan、neon、纯黑白、AI常用的冷灰蓝调、任何多色渐变背景（占位色块用纯色 `#FAFADE`，荷光笔高亮的 `linear-gradient(transparent 60%, #91BB7C 60%)` 保留），主视觉严格使用自有三色体系，新增色彩控制饱和度 |
